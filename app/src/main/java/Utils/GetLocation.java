...
public class GetLocation {
    ...
    public GetLocation(final Context context) {
        ...
        option.setProdName("placeInfo");                      //设置产品线名称
...
//                    sb.append("\ndescribe : ");
//                    sb.append("gps定位成功");
                    PromptManager.showToast(context,"gps定位成功");
...
//                    sb.append("\ndescribe : ");
//                    sb.append("网络定位成功");
                    PromptManager.showToast(context,"网络定位成功");
                } else if (location.getLocType() == BDLocation.TypeOffLineLocation) {// 离线定位结果
//                    sb.append("\ndescribe : ");
//                    sb.append("离线定位成功，离线定位结果也是有效的");
                    PromptManager.showToast(context,"离线定位成功");
                } else if (location.getLocType() == BDLocation.TypeServerError) {
//                    sb.append("\ndescribe : ");
//                    sb.append("服务端网络定位失败，可以反馈IMEI号和大体定位时间到loc-bugs@baidu.com，会有人追查原因");
                    PromptManager.showToast(context,"服务端网络定位失败");
                } else if (location.getLocType() == BDLocation.TypeNetWorkException) {
//                    sb.append("\ndescribe : ");
//                    sb.append("网络不同导致定位失败，请检查网络是否通畅");
                    PromptManager.showToast(context,"网络不同导致定位失败，请检查网络是否通畅");
                } else if (location.getLocType() == BDLocation.TypeCriteriaException) {
//                    sb.append("\ndescribe : ");
//                    sb.append("无法获取有效定位依据导致定位失败，一般是由于手机的原因，处于飞行模式下一般会造成这种结果，可以试着重启手机");
                    PromptManager.showToast(context,"无法获取有效定位依据导致定位失败，一般是由于手机的原因，处于飞行模式下一般会造成这种结果");
                }
//                sb.append("\nlocationdescribe : ");
...
//                    }
//                }
                dataFinishListener.dataFinishSuccessfully(sb.toString());
